...
## 1. 기존 노트북 활용
- `notebooks/05_모델_평가_및_해석.ipynb` (전체)
...
- **재무 관점 해석 추가**:
  ...
  #### True Positive (부도 기업을 부도로 예측): 346건
  - **비즈니스 가치**: 부도 위험 사전 차단
  - **예상 손실 회피**: 346 × 67백만원 = 232억원

  #### False Negative (부도 기업을 정상으로 예측): 410건
  - **비즈니스 리스크**: 부도 미탐지
  - **예상 손실**: 410 × 67백만원 = 275억원
  - **개선 필요**: FN 감소가 최우선 과제

  #### Type II Error: 53.95%
  - 부도 기업의 절반을 놓침
  - 원인: 데이터 외 정보 부족 (소송, 경영진 이슈 등)
  ```
...
- **대폭 강화된 한계 섹션 작성**:
  ...
  **검증 방안:**
  - 2022-2023 데이터로 시간적 검증 (Temporal Validation)
  - 다른 국가 데이터로 공간적 검증 (Spatial Validation)

  ### 5. 해석 가능성 vs 성능 트레이드오프 ⚖️

  **현재 선택:**
  - 도메인 특성 기반 → 해석 가능성 우선
  - XGBoost Single Model → 설명 용이

  **트레이드오프:**
  - Deep Learning 사용 시 성능 향상 가능 (예상 PR-AUC +0.02)
  - 하지만 설명 어려움 → 실무 적용 제한

  **결정 근거:**
  - 금융권 실무에서는 설명 가능성이 필수
  - 규제 요구사항 (Basel III, AI 윤리)
  - 심사역이 "왜 거절했는가?" 설명해야 함
  ...
  ### 4. 실용적 성능 향상
  - Naive Baseline 대비:
    * PR-AUC: 0.089 → 0.145 (63% 향상)
    * F2-Score: 0.14 → 0.21 (50% 향상)
  - 실제 손실 감소 효과 입증
...
### 달성한 것
1. ✅ 도메인 지식 기반 75개 특성 생성
2. ✅ 불균형 데이터 처리 (SMOTE+Tomek)
3. ✅ 최적 모델 선택 (XGBoost)
4. ✅ PR-AUC 0.145 (Baseline 대비 63% 향상)
5. ✅ 해석 가능한 예측 시스템

### 주요 한계
1. ⚠️ FN 53.95% (부도 기업 절반 놓침)
2. ⚠️ 시계열 데이터 부재
3. ⚠️ 외부 정보 미반영
